# Transferable 1-RDM / ML-OFDFT 현재 구현 정리

작성 기준: 2026-05-20. 이 문서는 현재 폴더에서 실제로 무엇을 계산하고, 어떤 데이터를 넣고, 모델이 어떤 수학적 구조로 gamma(r,r')를 예측하는지 설명한다.

## 1. 큰 그림

이 프로젝트의 목표는 3D real-space grid 위의 one-particle reduced density matrix, 즉 1-RDM gamma(r,r')를 직접 학습하는 것이다. 단순히 pair value 하나를 맞추는 regression이 아니라, gamma_theta를 중간 표현으로 두고 density rho, kinetic energy density tau, particle number, occupation spectrum, symmetry를 동시에 제어한다.
...
## 2. 현재 실제 데이터 흐름: QM9 구조 + PySCF DFT
...
## 3. AO density matrix에서 grid gamma_true를 만드는 방법

PySCF가 계산하는 DFT wavefunction은 real-space grid 위에 바로 저장되어 있지 않다. 대신 atomic orbital(AO) basis 위의 density matrix P를 얻는다. AO basis function을 chi_mu(r)라고 쓰면, 각 grid point r_g에서 AO 값을 평가하여 행렬 B를 만든다.

$$
B_{g\mu} = \chi_\mu(\mathbf{r}_g)
$$

closed-shell RKS에서 density matrix는 개념적으로 occupied molecular orbital coefficient C와 occupation에서 온다. PySCF에서는 mf.make_rdm1()이 이 P를 만들어 준다.
...
여기서 Gamma_true는 코드의 gamma_matrix다. 대각 원소는 density다.

$$
\rho_{\mathrm{true}}(\mathbf{r}_g)=\gamma_{\mathrm{true}}(\mathbf{r}_g,\mathbf{r}_g)
$$

변환기는 grid 적분 trace가 전자수 N과 맞도록 gamma_matrix를 한 번 스케일한다. 거친 7^3 grid에서 trace error를 줄이기 위한 실용적 보정이다.
...
- 000 train dsgdb9nsd_000003  H2O  3 atoms  10 e  O
- 001 train dsgdb9nsd_000005  HCN  3 atoms  14 e  C#N
- 002 train dsgdb9nsd_000002  H3N  4 atoms  10 e  N
- 003 train dsgdb9nsd_000004  H2C2  4 atoms  14 e  C#C
- 004 train dsgdb9nsd_000006  H2CO  4 atoms  16 e  C=O
- 005 train dsgdb9nsd_000025  C2N2  4 atoms  26 e  N#CC#N
- 006 train dsgdb9nsd_000001  H4C  5 atoms  10 e  C
- 007 train dsgdb9nsd_000024  HC3N  5 atoms  26 e  C#CC#N
- 008 train dsgdb9nsd_000027  HC2NO  5 atoms  28 e  O=CC#N
- 009 train dsgdb9nsd_000177  N4O  5 atoms  36 e  O1N=NN=N1
- 010 train dsgdb9nsd_000184  CF4  5 atoms  42 e  FC(F)(F)F
- 011 train dsgdb9nsd_000008  H4CO  6 atoms  18 e  CO

12개 smoke demo에 들어간 분자 예시는 다음과 같다.

- 0000_dsgdb9nsd_000184: CF4, 5 atoms, 42.0 e, SMILES=FC(F)(F)F
- 0001_dsgdb9nsd_000005: HCN, 3 atoms, 14.0 e, SMILES=C#N
- 0002_dsgdb9nsd_000486: C4N2, 6 atoms, 38.0 e, SMILES=N#CC#CC#N
- 0003_dsgdb9nsd_000024: HC3N, 5 atoms, 26.0 e, SMILES=C#CC#N
- 0004_dsgdb9nsd_000025: C2N2, 4 atoms, 26.0 e, SMILES=N#CC#N
- 0005_dsgdb9nsd_000003: H2O, 3 atoms, 10.0 e, SMILES=O
- 0006_dsgdb9nsd_000779: CN2O3, 6 atoms, 44.0 e, SMILES=O=C1ON=NO1
- 0007_dsgdb9nsd_000008: H4CO, 6 atoms, 18.0 e, SMILES=CO
- 0008_dsgdb9nsd_000323: C3N2O, 6 atoms, 40.0 e, SMILES=O=C(C#N)C#N
- 0009_dsgdb9nsd_000004: H2C2, 4 atoms, 14.0 e, SMILES=C#C
- 0010_dsgdb9nsd_000827: C2NF3, 6 atoms, 46.0 e, SMILES=FC(F)(F)C#N
- 0011_dsgdb9nsd_000012: H3CNO, 6 atoms, 24.0 e, SMILES=NC=O

## 6. NPZ schema와 차원

axis_points=7인 한 분자 system은 다음 배열을 가진다. 여기서 343은 7^3이고, full pair 수는 343^2 = 117,649개다.
...
학습 target의 중심은 gamma_matrix다. rho, tau, trace, occupation penalty는 이 gamma_matrix와 모델 예측으로부터 다시 계산된다.

## 7. local point feature xi(r): 15차원
...
## 10. 전체 모델은 세 개의 submodel로 구성된다

현재 ModelBundle은 point_model, pair_model, context_model 세 개로 구성된다. 세 모델은 모두 Random Fourier Features(RFF)로 입력을 확장한 뒤 SiLU activation을 쓰는 MLP다.
...
pair model은 eta(r,r')와 c를 받아 Gaussian baseline의 폭 alpha_theta와 residual correction을 얼마나 섞을지 정하는 scalar gate m_theta를 출력한다.
...
이전 문서에서는 g_theta라는 이름이 빠져 있거나 gate와 혼동될 수 있었다. 여기서는 명확히 분리한다. g_theta는 scalar gate가 아니라, 각 grid point를 normalized latent vector로 보내는 embedding이다.

point model의 residual mode amplitude를 a_theta(r;c)라고 하고, constant anchor channel을 붙인 feature를 phi_theta라고 하자.
...
context model이 만든 positive mode weight w(c)를 element-wise로 곱하고 L2 normalization하면 g_theta가 된다.
...
코드 변수로는 unit_r, unit_rp가 g_theta에 해당하고, residual_kernel이 위 내적에 해당한다. 반면 코드 변수 gate는 scalar m_theta다. 이 둘은 서로 다른 개념이다.

## 12. K_theta와 gamma_theta의 의미

gamma_theta는 density amplitude와 normalized kernel의 곱으로 쓴다. K_theta는 density scale을 제거한 뒤 남는 off-diagonal coherence를 표현한다.

$$
\gamma_\theta(\mathbf{r},\mathbf{r}';c)=\sqrt{\rho_\theta(\mathbf{r};c)\rho_\theta(\mathbf{r}';c)}\,K_\theta(\mathbf{r},\mathbf{r}';c)
$$

baseline kernel은 pair model이 예측한 alpha_theta를 사용한 Gaussian decay다.

$$
K_{\mathrm{base},\theta}(\mathbf{r},\mathbf{r}';c)=\exp\left[-\alpha_\theta(\mathbf{r},\mathbf{r}';c)\|\mathbf{r}-\mathbf{r}'\|^2\right]
$$

최종 kernel은 baseline과 residual을 scalar gate m_theta로 섞는다.

$$
K_\theta=K_{\mathrm{base},\theta}\left[(1-m_\theta)+m_\theta K_{\mathrm{res},\theta}\right]
$$

해석은 다음과 같다. K_base는 가까운 pair에서 기본적인 decay를 주는 물리적 bias다. K_res는 orbital-like nonlocal coherence correction이다. m_theta는 이 pair에서 residual correction을 얼마나 믿을지 정하는 adaptive switch다.

대각에서는 K_theta(r,r) ~= 1이 되어야 gamma_theta(r,r) = rho_theta(r)가 된다. 그래서 loss에 kernel diagonal normalization을 넣는다.
...
molecular closed-shell DFT density matrix의 natural occupation 범위는 0에서 2다. 그래서 QM9/PySCF 학습에서는 RDM_OCC_MAX=2.0을 쓰는 것이 맞다.

## 14. 학습 batch는 어떻게 만들어지는가
